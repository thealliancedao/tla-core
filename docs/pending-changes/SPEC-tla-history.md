# SPEC — `tla-history` (governance event capture: votes, locks, bribes, rewards)
...
| Decision | Default |
|---|---|
| tla-core module path | `history/events/` (module `history`, product `events`) |
...
| Forward | Render cron `org-tla-history` in `platform-crons/history/` (shares the seed's classifier byte-identical) |
...
| Contract | Owner |
|---|---|
| Gauge controller `terra1hfksrhchkmsj4qdq33wkksrslnfles6y2l77fmmzeep0xmq24l2smsd3lj` | **tla-history** |
| Voting escrow / vAMP minter `terra1uqhj8agyeaz8fu6mdggfuwr3lp32jlrx5hqag4jxexde92rzkamq3l62zg` | **tla-history** |
| Incentive manager `terra1tuuwm8yrj54qeg0c8xu00aha9ryatyhtczq8qq2q8tntuw0auzas9037wh` | **tla-history** |
...
Seed code: `tla-core/.github/scripts/tla-history/` +
`.github/workflows/tla-history-backfill.yml` (template: `flows-backfill.yml`;
inputs: mode sample|full, LCDs, pager knobs — carried over from the proven
workflow). Forward code: `platform-crons/history/` (README + package.json +
cron js; env `GITHUB_TOKEN` scoped to tla-core; schedule `0 */6 * * *`).
Changelog: `tla-core/docs/changelogs/cron-tla-history-log.md`.
...
5. Old repo parallel-runs until site verification (Batches 3–4), then
   retire `tla-history-data_2026` (frozen not deleted).
