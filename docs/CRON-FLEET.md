...
| **org-nft-flows-adao** | `nfts/nft-flows/` · index.js (1.1.4), env `COLLECTION=adao` | hourly `4 * * * *` | on-chain event ledger → `nft-collections/adao/{ledger,raw/forward,nft-flows/heartbeat.json}` (block cursor) | app NFTs tab (next), explorer per collection (queued) |
