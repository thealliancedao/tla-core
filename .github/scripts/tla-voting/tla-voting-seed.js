// =============================================================================
// tla-voting-seed.js — TLA VOTING event capture: votes, locks, bribes, rewards
...
// Lives in: tla-core/.github/scripts/tla-voting/   (org seed Action)
...
//   • Output → tla-core `tla-voting/events/` (module/product layout): the four
//     event streams + rollups + cursor + heartbeat + index. Built "as if the
//     Render cron ran all along" — org-tla-voting forward-maintains from
//     cursor.json.
...
// One-contract-one-owner: these three belong to the voting cron (spec §1).
...
const OUT_DIR       = 'tla-voting/events'; // module/product (spec §4)

const EPOCH_DATES_URL = `https://raw.githubusercontent.com/${GITHUB_REPO}/${GITHUB_BRANCH}/docs/epoch_1-300_date.json`;

// PATH MIGRATION (one-time, 2026-07-08): the module was renamed history/ →
// tla-voting/ (briefly voting/, possibly never published). Prior reads fall
// back through the old paths so the rename is lossless (the retention floor
// slides ~14k blocks/day — re-scanning from scratch would drop the earliest
// captured reward/bribe events). Once tla-voting/ is published, the fallbacks
// never fire again; delete tla-core/history/ (and voting/ if present) after.
const OLD_OUT_DIRS = ['voting/events', 'history/events'];
...
function httpGet(url, t = 20000) {
    return new Promise((res, rej) => {
        const r = https.get(url, { agent: KEEPALIVE_AGENT, headers: { Accept: 'application/json', Connection: 'keep-alive', 'User-Agent': 'tla-voting-seed/3.0' } }, (x) => {
...
function githubApiRequest(method, apiPath, body) {
    return new Promise((resolve, reject) => {
        const opts = { hostname: 'api.github.com', path: apiPath, method, headers: { 'User-Agent': 'tla-voting-seed', 'Authorization': `Bearer ${GITHUB_TOKEN}`, 'Accept': 'application/vnd.github+json' } };
...
async function getPrior(file, label) {
    const cur = await tryGetJson(RAW(file), `prior ${label}`);
    if (cur) return cur;
    for (const dir of OLD_OUT_DIRS) {
        const old = await tryGetJson(`https://raw.githubusercontent.com/${GITHUB_REPO}/${GITHUB_BRANCH}/${dir}/${file}?t=${Date.now()}`, `prior ${label} (${dir})`);
        if (old) { console.log(`   ↪ ${label}: migrated prior from ${dir} (one-time rename fallback)`); return old; }
    }
    return null;
}
...
async function publishHeartbeat(h) {
    const hb = {
        schemaVersion: SCHEMA_VERSION, cron: 'tla-voting', product: 'events',
...
async function run() {
    ...
    const discovered = {};
    console.log(`\n📜 tla-voting-seed — ${startedAt.toISOString()}${ARCHIVE_LCD ? `  [ARCHIVE MODE: ${ARCHIVE_LCD}]` : ''}\n`);
    ...
    const [priorVotes, priorLocks, priorBribes, priorRewards, epochDates] = await Promise.all([
        ...
        getPrior('lock-events.json', 'lock-events'),
        getPrior('bribe-events.json', 'bribe-events'),
        getPrior('reward-events.json', 'reward-events'),
    ...
    await publishFile(`${OUT_DIR}/vote-events.json`,   voteFile,   `tla-voting ${runMode}: votes ${vm.merged.length} (+${vm.added})`);
    await publishFile(`${OUT_DIR}/lock-events.json`,   lockFile,   `tla-voting ${runMode}: locks ${lm.merged.length} (+${lm.added})`);
    await publishFile(`${OUT_DIR}/bribe-events.json`,  bribeFile,  `tla-voting ${runMode}: bribes ${bm.merged.length} (+${bm.added})`);
    await publishFile(`${OUT_DIR}/reward-events.json`, rewardFile, `tla-voting ${runMode}: rewards ${rm.merged.length} (+${rm.added})`);
    await publishFile(`${OUT_DIR}/rollups.json`,       rollups,    `tla-voting rollups: ${rollups.wallet_count} wallets`);
