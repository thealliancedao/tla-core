...
function mergeMonth(existing, incoming){ const m=new Map(); for(const r of existing) m.set(r.txhash,r); let added=0, upgraded=0;
  // schema-upgrade merge (v2 re-fill, 2026-07-23): higher-schema record for a
  // known tx replaces in place; lower/equal never overwrites (idempotent).
  for(const r of incoming){ const p=m.get(r.txhash); if(!p){ m.set(r.txhash,r); added++; } else if(Number(r.schemaVersion||1)>Number(p.schemaVersion||1)){ m.set(r.txhash,r); upgraded++; } }
  return { merged:[...m.values()].sort((a,b)=>a.height-b.height||(a.txhash<b.txhash?-1:1)), added, upgraded }; }
...
async function main() {
  ...
  for (const mk of Object.keys(byMonth).sort()) {
    ...
    if (!added && !upgraded) { console.log(`  ${mk}: all duplicates — skip`); continue; }
    await putJson(p, merged, `gapfill ${mk}: +${added}, ${upgraded || 0} upgraded to v2 (${merged.length} total)`);
    published += added; console.log(`  ${mk}: +${added} · ↑${upgraded || 0} → ${merged.length}`);
  }
  // index totals
  if (published) {
